public class FreeBlock {
    ...
    public Node bestFit( int recordSize ) {
        ...
        while ( ( temp = temp.next() ) != tail ) {
            if ( recordSize <= temp.getBlockSize() && ( temp.getBlockSize() < fitBlock ) ) {
                best = temp;
    ...
    public int updateList( int recordSize ) {
        ...
        if ( best == null ) {
            ...
            if ( recordSize == best.getBlockSize() ) {
                ...
                // update the numberOfRecords and position
     ...
     *
     * @param node that will need to right || left
     */
     ...
     *
     * @param node that needs to check whether it is needed to merge
     *
    ...
    public boolean isMergeNeed( Node node ) {
        ...
        boolean merge;
        if ( node.getPosition() != node.prev().getPosition() + node.prev().getBlockSize() && node.next().getPosition() != node.getPosition()+ node.getBlockSize() ) {
            merge = false;
        } else {
            merge = true;
        }
